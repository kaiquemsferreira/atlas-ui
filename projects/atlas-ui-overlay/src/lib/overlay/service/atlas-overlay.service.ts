...
export class AtlasOverlayService {
  ...
  open<TComponent, TResult = unknown>(component: Type<TComponent>, config: AtlasOverlayConfig = {}): AtlasOverlayRef<TResult> {
    return this._open<TResult>({ kind: 'component', component: component as Type<any> }, config);
  }

  openTemplate<TData = unknown, TResult = unknown>(template: TemplateRef<unknown>, config?: {
  ...
  private _open<TResult>(content: | { kind: 'component'; component: any } | { kind: 'template'; template: TemplateRef<unknown> },
    ...
    if (content.kind === 'component') {
      contentRef = createComponent(content.component, {
        ...
        elementInjector: config.injector ?? this.injector,
      });
  ...
  private ensureContainer(): void {
    ...
    if (!el) {
      ...
      const themeHost = document.querySelector('[data-atlas-theme]') ??
        document.querySelector('.atlas-theme') ?? document.body;
